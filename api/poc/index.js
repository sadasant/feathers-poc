...
async function init() {
  ...
  while (true) {
    try {
      ...
      let response = _.get('data.0.children.1.context.response', await Search.find({ query: searchQuery }))
      console.log(colors.cyan('Search Response: %s'), JSON.stringify(response))
      ...
      console.log(colors.red('Failed running everything. Probably disconnected.'), e)
    }
